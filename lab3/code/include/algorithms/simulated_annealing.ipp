...
template<typename T>
SimulatedAnnealing<T>::SimulatedAnnealing(uint32_t hamming_distance, double initial_temperature, std::function<double(double)> cooling_schedule, uint64_t max_evals) :
...
template<typename T>
void SimulatedAnnealing<T>::evaluate(const std::vector<Node<T>*>&)
{
	if (m_first)
	{
		return;
...
template<typename T>
bool SimulatedAnnealing<T>::shouldTerminate(const std::vector<Node<T>*>&) const
{
	return m_should_terminate;
}

template<typename T>
void SimulatedAnnealing<T>::getNeighbors(const std::vector<Node<T>*>& nodes, NeighborGenerator<T>& gen, std::vector<Node<T>*>& neighbors)
{
	if (m_first)
	{
		m_first = false;
...
template<typename T>
void SimulatedAnnealing<T>::chooseNodes(std::vector<Node<T>*>& nodes, const std::vector<Node<T>*>& neighbors)
{
	Node<T>* node = nodes[0];
...
template<typename T>
uint64_t SimulatedAnnealing<T>::restarts() const
{
	return 0;
}

template<typename T>
uint64_t SimulatedAnnealing<T>::evaluations() const
{
